...
2. Run scope-appropriate local validation from `AGENTS.md` before pushing.
   At minimum, run the narrowest relevant test command and
   `Rscript dev/test-timing.R`; use `devtools::check()` for broad package,
   metadata, dependency, example, or vignette changes.
...
7. Re-read the rendered PR body and verify every template section is complete,
   all placeholder comments are removed, and validation claims match commands
   actually run.
...
# Scope-appropriate validation gate; choose commands from AGENTS.md.
