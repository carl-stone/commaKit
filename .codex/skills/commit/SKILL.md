...
2. Inspect the working tree and staged changes (`git status`, `git diff`,
   `git diff --staged`).
3. Stage intended paths by name after confirming scope. Do not use broad
   `git add -A` or `git add .` commands that can capture unrelated work,
   credentials, generated bulk, or local artifacts.
...
8. Write a body that includes:
   ...
   - A closing keyword (`Closes #N`, `Fixes #N`, or `Resolves #N`) when the
     commit fully implements a linked GitHub issue.
