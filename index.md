...
call needs a named vector of per-sample files, a sample metadata table,
genome lengths or a FASTA/BSgenome, and optionally annotation and motif
information:
...
uses M-values internally rather than raw beta values, which is more
...
maps 1-bp methylation sites to genomic features with
[`GenomicRanges::findOverlaps()`](https://rdrr.io/pkg/IRanges/man/findOverlaps-methods.html).
It keeps all associations in list-columns rather than collapsing each
site to a single closest feature.
...
![Mean methylation across gene
bodies.](reference/figures/README-plot-metagene-1.png)
...
![Heatmap of top differentially methylated 6mA
...
when site-level methylation should be aggregated over features, windows,
or other genomic intervals.
...
Export browser-compatible methylation tracks with
...
runs GO or KEGG enrichment on genes associated with differentially
methylated sites. The bundled example data use synthetic gene IDs, so
real biological enrichment examples require real bacterial annotations
...
- [`vignette("getting-started", package = "commaKit")`](https://carl-stone.github.io/commaKit/articles/getting-started.md) -
  end-to-end analysis from example data.
- [`vignette("understanding-commaData", package = "commaKit")`](https://carl-stone.github.io/commaKit/articles/understanding-commaData.md) -
  object model, assays, provenance, and result layers.
- [`vignette("import-troubleshooting", package = "commaKit")`](https://carl-stone.github.io/commaKit/articles/import-troubleshooting.md) -
  modkit, Dorado, Megalodon, genome, and annotation input issues.
- [`vignette("multiple-modification-types", package = "commaKit")`](https://carl-stone.github.io/commaKit/articles/multiple-modification-types.md) -
