...
test_that("siteInfo() contains required columns", {
  ...
  si <- siteInfo(obj)
  expect_true(
    all(c("chrom", "position", "strand", "mod_type") %in% colnames(si))
  )
})

test_that(
  paste(
    "siteInfo() site_key always includes chromosome for",
    "single-chromosome objects"
  ),
  {
    obj <- .make_two_modtype()
    si <- siteInfo(obj)
...
test_that("comma_example_data loads and accessors work correctly", {
  skip_if_not(
    ...
    message = paste0(
      "comma_example_data not yet generated — ",
      "run data-raw/create_example_data.R"
    )
  )
  ...
  expect_equal(
    as.character(samples$condition),
  ...
  expect_equal(
    range(siteCoverage(comma_example_data), na.rm = TRUE),
    c(10, 150)
  )
})
...
test_that("modContexts: works even without mod_context in mcols", {
  ...
  expect_false(
    "mod_context" %in% colnames(GenomicRanges::mcols(rowRanges(obj)))
  )
