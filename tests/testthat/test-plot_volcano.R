...
test_that(
  paste(
    "plot_volcano: p$data contains exact non-NA padj rows with",
    "computed neg_log10_padj"
  ),
  {
...
test_that("plot_volcano: exact zero padj produces finite plotted y values", {
  ...
  expect_equal(
    p$data$neg_log10_padj[zero_idx],
...
test_that("plot_volcano: significance categories match threshold rules", {
  ...
  expect_true(
    any(hyper),
    "Fixture should produce at least one hypermethylated site"
  )
  expect_equal(
    as.character(pd$significance[hyper]),
    rep("Hypermethylated", sum(hyper))
  )
  # Rows with padj <= 0.01 and delta_beta <= -0.3 should be "Hypomethylated"
  hypo <- pd$dm_padj <= 0.01 & pd$dm_delta_beta <= -0.3
  expect_true(
    any(hypo),
    "Fixture should produce at least one hypomethylated site"
  )
  expect_equal(
    as.character(pd$significance[hypo]),
    rep("Hypomethylated", sum(hypo))
  )
})
...
test_that("plot_volcano: error on non-data.frame input", {
  expect_error(
    plot_volcano(list(dm_delta_beta = 1, dm_padj = 0.01)),
    "data.frame"
  )
})
...
test_that("plot_volcano: NULL delta_beta_threshold colors by padj alone", {
  ...
  expect_true(
    any(sig_pos),
    "Fixture should produce sig_pos sites at padj <= 0.05"
  )
  expect_equal(
    as.character(pd$significance[sig_pos]),
    rep("Hypermethylated", sum(sig_pos))
  )
  sig_neg <- pd$dm_padj <= 0.05 & pd$dm_delta_beta < 0
  expect_true(
    any(sig_neg),
    "Fixture should produce sig_neg sites at padj <= 0.05"
  )
  expect_equal(
    as.character(pd$significance[sig_neg]),
    rep("Hypomethylated", sum(sig_neg))
  )
})
...
test_that("plot_volcano: error on invalid delta_beta_threshold", {
  res <- .make_volcano_results()
  expect_error(
    plot_volcano(res, delta_beta_threshold = 0),
    "delta_beta_threshold"
  )
  expect_error(
    plot_volcano(res, delta_beta_threshold = 1),
    "delta_beta_threshold"
  )
  expect_error(
    plot_volcano(res, delta_beta_threshold = -0.1),
    "delta_beta_threshold"
  )
})
