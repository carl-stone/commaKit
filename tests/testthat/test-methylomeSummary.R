...
test_that("methylomeSummary: has required columns", {
  ...
  result <- methylomeSummary(comma_example_data)
  expected_cols <- c(
    "sample_name", "condition", "mod_type", "n_sites",
...
test_that(
  "methylomeSummary: beta and coverage summaries use documented inputs",
  {
    data(comma_example_data)
...
test_that("methylomeSummary: error on non-commaData input", {
  expect_error(
    methylomeSummary(data.frame(x = 1)),
...
test_that("methylomeSummary: error on unknown mod_type", {
  data(comma_example_data)
  expect_error(
    methylomeSummary(comma_example_data, mod_type = "9mX"),
    "not found in object"
  )
})
...
test_that(
  paste(
    "methylomeSummary: all-NA sample gives NA mean_beta and",
    "median_beta, not error"
  ),
  {
    data(comma_example_data)
    ...
    se_mod <- SummarizedExperiment::SummarizedExperiment(
      assays = list(
        methylation = methyl,
        coverage = siteCoverage(comma_example_data)
      ),
...
test_that("methylomeSummary: min_coverage column is present", {
  data(comma_example_data)
