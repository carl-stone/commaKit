test_that("assayLayers() lists core assays and inferred defaults", {
  ...
  expect_equal(
    layers$type[layers$assay == "mod_counts"],
    "reconstructed_counts"
  )
  expect_equal(
    layers$type[layers$assay == "other_mod_counts"],
    "observed_counts"
  )
})
...
test_that(
  paste(
    "commaKit:::.addAssayLayer() adds named derived layers without",
    "replacing raw assays"
  ),
  {
    obj <- .make_two_modtype_fixture()
    ...
    expect_true(
      "methylation_mvalue.alpha05" %in%
        SummarizedExperiment::assayNames(obj2)
    )
    ...
    expect_equal(
      as.character(added$parent_assays[[1L]]),
...
test_that(
  paste(
    "commaKit:::.addAssayLayer() protects existing layers unless",
