...
test_that("coverageDepth: has required columns", {
  ...
  expect_true(
    all(
      c("chrom", "window_start", "window_end", "sample_name", "depth") %in%
        colnames(result)
    )
  )
})
...
test_that("coverageDepth: log2_depth is log2(depth+1)", {
  ...
  expect_equal(
    result$log2_depth[non_na],
    log2(result$depth[non_na] + 1),
    tolerance = 1e-10
  )
})
...
test_that(
  paste(
    "coverageDepth: method='median' and 'mean' both return valid",
    "results with correct row counts"
  ),
  {
    data(comma_example_data)
    result_median <- coverageDepth(
      comma_example_data,
      window = 10000L,
      method = "median"
    )
    result_mean <- coverageDepth(
      comma_example_data,
      window = 10000L,
      method = "mean"
    )
    ...
    # Same sample names
    expect_equal(
      sort(unique(result_median$sample_name)),
...
test_that("coverageDepth: error on non-commaData input", {
  expect_error(
    coverageDepth(data.frame(x = 1), window = 1000),
