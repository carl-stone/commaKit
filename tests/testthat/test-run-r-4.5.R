test_that("R version hint matches renv metadata", {
  ...
  settings <- readLines(
    file.path(project_root, "renv", "settings.json"),
    warn = FALSE
  )
...
test_that("run-r-4.5 creates an overlay Rscript that uses overlay R", {
  ...
  overlay_home <- file.path(
    tempdir(),
    paste0("commakit-r-4.5-home.", sample.int(1e6, 1))
  )
  ...
  expect_true(
    any(grepl("--no-echo --no-restore -e", rscript_result, fixed = TRUE))
  )
  ...
  expect_true(
    any(grepl(
      "--args vignettes/getting-started.Rmd",
      rscript_expr_args,
      fixed = TRUE
    ))
  )
