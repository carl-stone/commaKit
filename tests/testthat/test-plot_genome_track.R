...
test_that(
  paste(
    "plot_genome_track: mod_type filter to 6mA produces identical",
    "data (all sites are 6mA)"
  ),
  {
    obj <- .make_track_data()
...
test_that("plot_genome_track: error on non-commaData input", {
  expect_error(
    plot_genome_track(data.frame(), chromosome = "chr_sim"),
    "commaData"
  )
})
...
test_that("plot_genome_track: error when chromosome not in genome", {
  obj <- .make_track_data()
  expect_error(
    plot_genome_track(obj, chromosome = "chrX"),
    "chrX"
  )
})
...
test_that("plot_genome_track: error when no sites on chromosome", {
  ...
  expect_error(
    plot_genome_track(obj, chromosome = "chr2"),
