...
test_that("summarizeRegions() validates inputs", {
  ...
  expect_error(
    summarizeRegions(obj, .make_regions(), min_sites = -1),
    "min_sites"
  )
})
...
test_that("summarizeRegions() validates motif and mod_context filters", {
  ...
  expect_error(
    summarizeRegions(obj, regions, motif = "BAD"),
    "motif.*not found"
  )
  expect_error(
    summarizeRegions(obj, regions, mod_context = "6mA_BAD"),
...
test_that("summarizeRegions() rejects non-integer min_sites", {
  obj <- .make_region_summary_fixture()
  expect_error(
    summarizeRegions(obj, .make_regions(), min_sites = 1.5),
    "min_sites"
  )
})
