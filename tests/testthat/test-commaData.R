...
.make_minimal_commaData <- function(n_sites = 5L, n_samples = 2L) {
  ...
  cov <- matrix(
    as.integer(runif(n_sites * n_samples, 10, 50)),
    nrow = n_sites,
    ncol = n_samples,
    dimnames = list(NULL, paste0("s", seq_len(n_samples)))
  )
...
.constructor_modkit_row <- function(
  start = 99L,
  mod_code = "a,GATC,1",
...
.write_constructor_modkit_rows <- function(
  rows,
...
test_that("validity rejects unrecognized mod_type values", {
  ...
  mcols(rowRanges(obj))$mod_type <- factor(
    rep("7mX", nrow(obj)),
    levels = c("7mX")
  )
...
test_that("validity rejects rowRanges with width != 1", {
  ...
  IRanges::ranges(rr)[1, ] <- IRanges::IRanges(
    start = start(rr)[1],
    width = 10L
  )
  rowRanges(obj) <- rr
...
test_that("commaData() masks only known values below the min_coverage threshold", {
  rows <- data.frame(
    chrom = "chr_sim",
...
test_that("validity rejects raw count sums above coverage", {
  ...
  expect_error(
    validObject(obj),
    regexp = "mod_counts.*canonical_counts.*coverage"
  )
})

test_that(
  paste(
    "commaData() rejects duplicate parsed site identities before",
    "merge assignment"
  ),
  {
