...
test_that("slidingWindow: returns a data.frame", {
  result <- slidingWindow(tiny_data, window = window_size)
...
test_that("slidingWindow: output has required columns", {
  ...
  expect_true(
    all(
      c("chrom", "position", "sample_name", "window_median") %in%
        colnames(result)
    )
  )
})
...
test_that("slidingWindow: stat='mean' produces window_mean column", {
  result <- slidingWindow(tiny_data, window = window_size, stat = "mean")
...
test_that("slidingWindow: mod_type filtering works", {
  result_6ma <- slidingWindow(tiny_data, window = window_size, mod_type = "6mA")
  result_5mc <- slidingWindow(tiny_data, window = window_size, mod_type = "5mC")
...
test_that("slidingWindow: circular=FALSE returns correct row count with valid values", {
  result <- slidingWindow(tiny_data, window = window_size, circular = FALSE)
  expect_s3_class(result, "data.frame")
...
test_that("slidingWindow: circular boundary values match manual calculation", {
  ...
  circular_mean <- slidingWindow(
    obj,
    window = 3L,
    stat = "mean",
    circular = TRUE
  )
  linear_mean <- slidingWindow(
    obj,
    window = 3L,
    stat = "mean",
    circular = FALSE
...
test_that("slidingWindow: circular must be TRUE, FALSE, or NULL", {
  expect_error(
    slidingWindow(tiny_data, window = window_size, circular = NA),
    "TRUE, FALSE, or NULL"
  )
  expect_error(
    slidingWindow(tiny_data, window = window_size, circular = c(TRUE, FALSE)),
    "TRUE, FALSE, or NULL"
  )
})
...
test_that("slidingWindow: error on invalid mod_type", {
  expect_error(
    slidingWindow(tiny_data, window = window_size, mod_type = "invalid_type"),
    "not found in object"
  )
})
...
test_that("slidingWindow: known smoothed value for simple input", {
  ...
  cov_mat <- matrix(10L, nrow = 3, ncol = 1, dimnames = list(NULL, "samp1"))
  cd <- S4Vectors::DataFrame(
    sample_name = "samp1",
    condition = "ctrl",
    replicate = 1L
