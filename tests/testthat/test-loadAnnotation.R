...
test_that(
  paste(
    "loadAnnotation() warns and returns empty GRanges when",
    "feature_types has no matches"
  ),
  {
    skip_if_not_installed("rtracklayer")
    gff <- system.file("extdata", "example.gff3", package = "commaKit")
...
test_that(
  paste(
    "loadAnnotation() feature_type column contains GFF3 type",
    "column value, not attribute"
  ),
  {
    skip_if_not_installed("rtracklayer")
    f <- .write_sigma_gff3()
...
test_that(
  paste(
    "loadAnnotation() feature_subtype is absent when GFF3 has no",
