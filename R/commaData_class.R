#' @importFrom methods new setClass setGeneric setMethod setValidity
#' @importFrom methods validObject is isVirtualClass
...
setValidity("commaData", function(object) {
  ...
  if ("motif" %in% colnames(mc)) {
    if (!is.character(mc$motif) && !all(is.na(mc$motif))) {
      errors <- c(
        errors,
        paste0(
          "rowRanges mcols$motif must be a character vector ",
          "(NA allowed)"
        )
      )
    }
  ...
  if ("methylation" %in% assayNames(object)) {
    ...
    has_bad_methyl <- length(methyl_vals) > 0L &&
  ...
  count_assays_present <- all(
    c("mod_counts", "canonical_counts", "coverage") %in%
      assayNames(object)
  )
  if (count_assays_present) {
    ...
    too_large <- comparable &
      (mod_counts + canonical_counts + other_for_sum > cov)
    if (any(too_large)) {
      errors <- c(
        errors,
        paste0(
          "assays 'mod_counts' + 'canonical_counts' + 'other_mod_counts' ",
          "must be <= assay 'coverage'"
        )
      )
    }
  }
