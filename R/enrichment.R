...
.computeGeneScores <- function(site_gene_df, score_metric = "combined",
  ...
  df$site_score <- switch(score_metric,
    ...
    stop(
      "'score_metric' must be one of: \"combined\", \"padj\", \"delta_beta\""
    )
  )
    ...
    RNA_binding_site = {
      ...
      for (i in seq_len(n)) {
        ...
        if (grepl(gene_pat, first_word, perl = TRUE)) {
          gene <- paste0(
            tolower(substring(first_word, 1, 1)),
...
buildKEGGTermGene <- function(organism, file = NULL, strip_prefix = TRUE,
  ...
  if (length(path_list) > 0L) {
    ...
    # e.g. "Glycolysis / Gluconeogenesis - Escherichia coli K-12"
...
buildKEGGGeneIDMap <- function(organism,
  ...
  matched_symbol <- sym_df$symbol[match(
    kegg_conv_df$entrez_id,
    sym_df$entrez_id
  )]
...
enrichMethylation <- function(object,
  ...
  if (is.null(OrgDb) && is.null(organism) && is.null(TERM2GENE) &&
    ...
    stop(
      ...
      "  OrgDb          -- Bioconductor OrgDb object for GO ",
      "(e.g., org.EcK12.eg.db)\n",
  ...
  if (is(object, "commaData")) {
    ...
    if (!is.null(mod_type) || !is.null(mod_context)) {
      warning(
        ...
        "are ignored. Pre-filter the data.frame before calling ",
        "enrichMethylation()."
      )
    }
  }
  ...
  results_by_ft <- lapply(ft_loop, function(ft) {
    ...
    if (is.null(ft)) {
      ...
      val <- overlap_only[ft]
      eff_overlap_only <- if (is.na(val)) {
        ft %in% .COMMA_REGION_FEATURE_TYPES
      } else {
        val
      }
    } else {
