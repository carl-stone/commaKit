...
annotateSites <- function(object,
  ...
  if (!is(features, "GRanges")) {
    stop(
      "'features' must be a GRanges object (or NULL to use annotation(object))."
    )
  }
  ...
  meta_out_cols <- if (!is.null(metadata_cols)) {
    paste0(metadata_cols, "_values")
  } else {
    character(0)
  }
...
.annotateSites_unified <- function(rd,
  ...
  feat_types <- as.character(
    GenomicRanges::mcols(features)[[feature_col]][s_idx]
  )
  ...
  if (!is.null(metadata_cols)) {
    for (col in metadata_cols) {
      ...
      rd[[paste0(col, "_values")]] <- S4Vectors::splitAsList(
        meta_vals,
        site_factor
      )
    }
...
.expandSiteWindows <- function(sites_gr, window) {
  ...
  normal_end[sized_normal] <- pmin(
    normal_end[sized_normal],
...
.circularFeatureRelPosition <- function(site_pos, feat_start, feat_end,
  ...
  rel_pos[plus_like & upstream_plus] <-
    -dist_site_to_start[plus_like & upstream_plus]
  rel_pos[plus_like & !upstream_plus] <-
    dist_end_to_site[plus_like & !upstream_plus]

  upstream_minus <- dist_end_to_site <= dist_site_to_start
  rel_pos[!plus_like & upstream_minus] <-
    -dist_end_to_site[!plus_like & upstream_minus]
  rel_pos[!plus_like & !upstream_minus] <-
    dist_site_to_start[!plus_like & !upstream_minus]
