# commaKit Vision — The Dream Package

**What commaKit would look like if it were as good as DESeq2 for bacterial methylation.**
...
commaKit should match this standard for methylation analysis.
...
## 3. Feature Roadmap — Dream commaKit
...
| **Vignette: enrichment workflow** | The enrichment pipeline is the most complex part of commaKit and has no vignette coverage. | Extend getting-started vignette or add a third vignette. |
...
| **Multi-factor designs** | Time-course, strain x condition, dose-response. | Extend formula interface; ensure all backends handle >2 conditions. |
...
| **DM backends** | 3 (beta-binomial/quasi_f, methylKit, limma) | 2 (Fisher, logistic) | 1 (beta-binomial Wald) | 1 (beta-binomial) | Multiple |
...
**commaKit's gaps vs competition:**
...
2. **mod_context is never optional.** Every analysis must respect that 6mA:GATC and 6mA:ACCACC are biologically distinct. Mixing them is a scientific error.
...
How we know commaKit has arrived:
